...
if ( awp_ver( '3.0' ) )
	...
	$taxonomies = $GLOBALS['scoper']->taxonomies->get_all_keys();

foreach( $taxonomies as $taxonomy ) :
	$user_terms = array();
	$status = ( 'publish' == $post->post_status ) ? 'published' : $post->post_status;
	scoper_filter_terms_for_status( $taxonomy,  array(), $user_terms, array( 'object_id' => $post->ID, 'object_type' => $post->post_type, 'status' => $status ) );
...
<?php
	...
	function add_meta_boxes() {
		log_mem_usage_rs( 'add_meta_boxes' );
		...
		*/
		...
		log_mem_usage_rs( 'add_meta_boxes done' );
	}
	
	function act_tweak_metaboxes() {
		log_mem_usage_rs( 'tweak meta_boxes' );
		...
		log_mem_usage_rs( 'meta_boxes done' );
	}
	...
	function draw_object_roles_content( $object, $box ) {
		...
		// id format: src_name:object_type:role_handle
