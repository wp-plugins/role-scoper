...
if ( strpos($_SERVER['SCRIPT_NAME'], 'p-admin/index.php' ) && ! defined( 'USE_RVY_RIGHTNOW' )  ) {
	if ( awp_ver( '3.0' ) )
		...
		include_once( 'admin-dashboard-legacy_rs.php' );
}
...
class ScoperAdmin
{
	...
	function admin_head() {
		...
		log_mem_usage_rs( 'admin_head done' );
	}
	...
	function build_menu() {
		...
		*/
				...
				if ( $can_admin_objects ) {
					foreach ( $scoper->data_sources->get_all() as $src_name => $src ) {
						...
						foreach ( array_keys($object_types) as $object_type ) {
							...
							if ( $require_blogwide_editor ) {
								if ( ! $scoper->user_can_edit_blogwide( $src_name, $object_type, array( 'require_others_cap' => true ) ) )
									continue;
							}
		...
		log_mem_usage_rs( 'build_menu done' );
	}
