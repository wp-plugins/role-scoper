...
class ScoperAdmin
{
	...
	function flt_contextual_help_list ($help, $screen) {
		...
		if ( strpos( $screen, SCOPER_FOLDER . '/admin/' ) ) {
			$match = array();
			if ( ! preg_match( "/admin_page_" . SCOPER_FOLDER . "\/admin[^@]*/", $screen, $match ) )
				if ( ! preg_match( "/_page_" . SCOPER_FOLDER . "\/admin[^@]*/", $screen, $match ) )
					preg_match( "/" . SCOPER_FOLDER . "\/admin[^@]*/", $screen, $match );

			if ( $match )
				if ( $pos = strpos( $match[0], SCOPER_FOLDER . '/admin/' ) )
					$link_section = substr( $match[0], $pos + strlen(SCOPER_FOLDER . '/admin/') );
	...
	function build_menu() {
		...
		*/
		...
		if ( DEFINE_GROUPS_RS && $can_manage_groups ) {
			...
			if ( false !== strpos( $_SERVER['REQUEST_URI'], 'default_groups.php' ) )
				add_submenu_page('users.php', __('User Groups', 'scoper'), __('Default Groups', 'scoper'), $cap_req, SCOPER_FOLDER . '/admin/default_groups.php');
		}
		...
		if ( ! awp_ver('2.7-dev') && ! awp_is_plugin_active('wp_ozh_adminmenu.php') ) { 
			if ( strpos($uri, SCOPER_FOLDER . '/admin/restrictions/') )
				$roles_link = str_replace('/admin/restrictions/', '/admin/roles/', $uri);
			
			elseif ( strpos($uri, SCOPER_FOLDER . '/admin/roles/') )
		...
		if ( $is_administrator ) {
			...
			$_registered_pages['admin_page_' . SCOPER_FOLDER . '/admin/options'] = SCOPER_FOLDER . "/admin/options.php";
		}
			...
			if ( 'term' == $scope ) {
				...
				if ( $can_admin_terms ) {
					...
					foreach ( $this->scoper->taxonomies->get_all() as $taxonomy => $tx ) {
						...
						if ( ! $tweak_menu ) {
							if ( $first_pass ) {
								$roles_hook_page = ( $is_administrator || $general_roles ) ? $roles_hook_page_base : 'toplevel_page_';	// otherwise Options / General are the toplevel submenu
						...
						add_action($roles_hook_page . SCOPER_FOLDER . "/admin/roles/$taxonomy", create_function( '', $func ) );	
						
						if ( ! awp_ver('2.6') )
							add_action("_page_" . SCOPER_FOLDER . "/admin/roles/$taxonomy", create_function( '', $func ) );	
						...
						if ( ! empty($tx->requires_term) ) {
							...
							add_action($restrictions_hook_page . SCOPER_FOLDER . "/admin/restrictions/$taxonomy", create_function( '', $func ) );	
				
							if ( ! awp_ver('2.6') )
								add_action("_page_" . SCOPER_FOLDER . "/admin/restrictions/$taxonomy", create_function( '', $func ) );	
				...
				if ( $can_admin_objects ) {
					foreach ( $this->scoper->data_sources->get_all() as $src_name => $src ) {
						...
						foreach ( array_keys($object_types) as $object_type ) {
							...
							add_action($roles_hook_page . SCOPER_FOLDER . "/admin/roles/$src_name/$object_type", create_function( '', $func ) );
						
							if ( ! awp_ver('2.6') )
								add_action("_page_" . SCOPER_FOLDER . "/admin/roles/$src_name/$object_type", create_function( '', $func ) );
							
							$func = "include_once('$path' . '/admin/object_restrictions.php');scoper_admin_object_restrictions('$src_name', '$object_type');";
							add_action($restrictions_hook_page . SCOPER_FOLDER . "/admin/restrictions/$src_name/$object_type", create_function( '', $func ) );	
					
							if ( ! awp_ver('2.6') )
								add_action("_page_" . SCOPER_FOLDER . "/admin/restrictions/$src_name/$object_type", create_function( '', $func ) );	
		...
		if ( $is_administrator ) {
			...
			if ( isset($submenu[$roles_menu][0][2]) && ( $roles_menu == $submenu[$roles_menu][0][2] ) )
			 ...
			 // satisfy WordPress' demand that all admin links be properly defined in menu
			if ( false !== strpos( $_SERVER['REQUEST_URI'], 'attachments_utility.php' ) )
				add_submenu_page($roles_menu, __('Attachment Utility', 'scoper'), __('Attachment Utility', 'scoper'), 'read', SCOPER_FOLDER . '/admin/attachments_utility.php');
