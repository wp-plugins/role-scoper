...
function scoper_version_updated( $prev_version ) {
	...
	do {
		/*
		// change lock_top_pages option to array structure for custom types
		if ( version_compare( $prev_version, '1.2.9', '<') ) {
			if ( $ lock_top_pages = get_option( 'scoper_lock_top_pages' ) ) {
				if ( ! get_option( 'scoper_lock_top_level' ) )
					update_option( 'scoper_lock_top_level', array( 'page' => $lock_top_pages ) );
			}
		}
		*/
		...
		// 1.2.9.RC4 changed RS cache path to subfolder, so flush the root-stored cache one last time (only for MU / Multisite due to potentially large # of folders, files)
		if ( IS_MU_RS && version_compare( $prev_version, '1.2.9.RC4', '<') && ! defined( 'SKIP_CACHE_MAINT_RS' ) ) {
...
function scoper_set_default_rs_roledefs() {
	...
	$any_custom = $scoper->any_custom_registrations();

	$default_role_caps = scoper_core_role_caps($any_custom);
