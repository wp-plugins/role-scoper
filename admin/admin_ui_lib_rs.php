...
class ScoperAdminUI {
	...
	function dropdown_pages($object_id = '', $stored_parent_id = '') {
		...
		$all_pages = get_pages();
		$titles = scoper_buffer_property( $all_pages, 'ID', 'post_title' );
		...
		scoper_restore_property( $args['pages'], $titles, 'ID', 'post_title' );
	...
	function walk_parent_dropdown( &$output, $args, $use_parent_clause = true, $default = 0, $parent = 0, $level = 0 ) {
		...
		if ( $items ) {
			foreach ( $items as $item ) {
				...
				$output .= "\n\t<option " . $class . 'value="' . $item->ID . '"' . $current . '>' . $pad . wp_specialchars($item->post_title) . '</option>';
