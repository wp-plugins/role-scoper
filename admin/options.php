...
<span class="rs-subtext">
<?php if ( $display_hints) _e('Either role type can be scoped (assigned for specific terms or objects).  However, RS roles allow finer control.  Wordpress roles will be used as defaults regardless of this selection.', 'scoper') ?>
</span>
<br />
<?php
...
_e('Hidden Editing Elements', 'scoper') ?></th>
<td>
...
<?php
	if ( $display_hints) {
		echo ('<div class="agp-vspaced_input">');
		_e('For decluttering convenience, hide elements with these html ids from users who do not have full editing capabilities for the post/page. Separate with&nbsp;;', 'scoper');
		echo '</div>';
		echo ('<div class="agp-vspaced_input">');
		_e('Note that this is <strong>not a security precaution</strong>. These settings may still be editable by ambitious users.', 'scoper');
		echo '</div>';
	}
?>
</div>
