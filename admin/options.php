			...
			if ( isset($this->def_otype_options[$option_name]) ) {
				...
				foreach ( $return['val'] as $src_otype => $val ) {
					if ( $caption_header && $first_pass ) {
						printf( $caption, $scoper->admin->interpret_src_otype($src_otype) );  // translation back-compat
					...
					$item_label = $scoper->admin->interpret_src_otype($src_otype, $label_property); //arg: use plural display name
	...
	<?php
		...
		if ( isset($ui->def_otype_options[$option_basename]) ) {
			...
			foreach ( $do_teaser as $src_name => $val ) {
				...
				$type_caption = $scoper->admin->interpret_src_otype('post:post');
					...
					foreach ( $opt_vals as $key => $val ) {
						...
						if ( TERM_SCOPE_RS == $scope ) {
							if ( awp_ver( '3.0' ) ) {
								...
								$display_name = $scoper->taxonomies->member_property( $key, 'display_name' );	
							}	
						} else {
							if ( awp_ver( '3.0' ) ) {
								...
								$display_name = $scoper->data_sources->member_property( 'post', 'object_types', $key, 'display_name' );	
						}
