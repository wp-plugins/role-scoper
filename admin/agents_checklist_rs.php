...
define ('ELIGIBLE_ITEMS_RS', 'eligible');
 ...
 class ScoperAgentsChecklist {
	...
	function _agents_checklist_display( $agents_subset, $role_basis, $all_agents, $id_prefix, $stored_assignments, $args, &$key) {
		...
		if ( ! is_array($via_other_scope_ids) ) $via_other_scope_ids = array(); else $via_other_scope_ids = array_flip($via_other_scope_ids);
		...
		$any_inherited = $any_other_scope = $any_other_role = $any_other_basis = false;
		$agent_count = array();
		...
		if ( $agent_count[$agents_subset] > $filter_threshold ) {
			...
			echo "<b><label for='chk_{$agents_subset}_{$id_prefix}'>";
			...
			printf ("<div class='rs-agents_caption'><b>$caption</b></div>", $agent_count[$agents_subset]);
			echo '</li></ul>';
		}
		...
		if ( $any_display_filtering || $agent_count[$agents_subset] > $emsize_threshold ) {
			...
			//if ( ! $ems_per_character = get_option('scoper_ems_per_character') )
		...
		foreach( $all_agents as $agent ) {
			...
			if ( isset($via_other_role_ids[$id]) )
				$label_class = ' class="rs-via-r"';
				
			elseif ( ! $role_assigned && isset($via_other_basis_ids[$id]) )
				$label_class =  'class="rs-via-b"';
					
			elseif ( isset($via_other_scope_ids[$id]) )
				$label_class = ' class="rs-via-s"';
			else
				$label_class = '';
			
				
			echo "\r\n<li $li_title>"
				. "<input type='checkbox' name='{$id_prefix}[]'{$disabled}{$this_checked} value='$id' id='{$id_prefix}{$id}' />";
			...
			if ( $propagation ) {
				...
				echo "{"
					. "<input type='checkbox' name='p_{$id_prefix}[]'{$disabled}{$this_checked_prop} value='$id' id='p_{$id_prefix}{$id}' />"
					. "}";
			}
			
			echo "<label for='{$id_prefix}{$id}'{$label_class}>";
		...
		if ( $any_inherited && $inherited_prefix )
			$key ['inherited']= "$inherited_prefix $inherited_suffix"
				 . '<span class="rs-keytext">' . sprintf(_c('inherited from parent %s|user/group role status key: this role is assigned via parent object', 'scoper'), strtolower($objtype_display_name)) . '</span>';
		
		if ( $any_other_role && $via_other_role_prefix )
			$key ['other_role']= "<span class='rs-via-r'>{$via_other_role_prefix}&nbsp;{$via_other_role_suffix}"
				 . '<span class="rs-keytext">' . str_replace( ' ', '&nbsp;', _c('has via other role|user/group role status key: all caps in this role are assigned via another role', 'scoper') ) . '</span></span>';
		...
		if ( $any_other_basis && $via_other_basis_prefix )
			$key ['other_basis']= "<span class='rs-via-b'>{$via_other_basis_prefix}&nbsp;{$via_other_basis_suffix}"
				 . '<span class="rs-keytext">' . str_replace( ' ', '&nbsp;', _c('has via group|user role status key: this role is assigned to a group the user is in', 'scoper') ) . '</span></span>';
		...
		if ( $any_other_scope && $via_other_scope_prefix )
			$key ['other_scope']= "<span class='rs-via-s'>{$via_other_scope_prefix}&nbsp;{$via_other_scope_suffix}"
				 . '<span class="rs-keytext">' . str_replace( ' ', '&nbsp;', _c('has via other scope|user role status key: this role is assigned blog-wide or term-wide', 'scoper') ) . '</span></span>';
		 
		if ( $propagation )
			$key ['propagation']= "{<input type='checkbox' disabled='disabled' name='rs-prop_key_{$agents_subset}_{$id_prefix}' id='rs-prop_key_{$agents_subset}_{$id_prefix}' />}"
				 . '<span class="rs-keytext">' . sprintf(_c('propagate to sub-%s|user/group role status key: propagate this role to sub-objects', 'scoper'), strtolower($objtype_display_name_plural)) . '</span>';
	}
