...
if( basename(__FILE__) == basename($_SERVER['SCRIPT_FILENAME']) )
		...
		foreach( $post_type_obj->taxonomies as $taxonomy ) {
			...
			if ( $taxonomy_obj = get_taxonomy($taxonomy) ) {
				...
				if ( $tags ) {
					...
					$cap_name = ( awp_ver( '3.0' ) ) ? $taxonomy_obj->cap->assign_terms : 'edit_posts';
	...
	function scoper_mnt_save_object($src_name, $args, $object_id, $object = '') {
		log_mem_usage_rs( 'scoper_mnt_save_object' );
		
		// operations in this function only apply to main post save action, not revision or auto-draft save
		if ( 'post' == $src_name ) {
		...
		// Determine whether this object is new, and whether its parent setting has changed since the last application of this filter
		if ( 'post' == $src_name ) {
			$is_new_object = empty( $scoper->filters_admin->logged_post_update[$object_id] ) && ( empty($scoper->filters_admin->last_post_status[$object_id]) || ( 'new' == $scoper->filters_admin->last_post_status[$object_id] ) || ( 'auto-draft' == $scoper->filters_admin->last_post_status[$object_id] ) );
		...
		if ( $new_role_settings && ! empty($_POST['rs_object_roles']) && ( empty($_POST['action']) || ( 'autosave' != $_POST['action'] ) ) && ! defined('XMLRPC_REQUEST') ) {
			...
			$require_blogwide_editor = scoper_get_option('role_admin_blogwide_editor_only');		// NOTE: this is redundant since assign_roles() and restrict_roles() also check it (but leave as an inexpensive extra safeguard)
			if ( 
		...
		if ( ( 'post' == $src_name ) && awp_ver('3.0') ) {
			$type_obj = get_post_type_object( $object_type );
			$do_flush = ! empty( $type_obj->hierarchical );
		} else
			$do_flush = ( 'post' != $object_type );
		...
		log_mem_usage_rs( 'scoper_mnt_save_object done' );	
	}

	
	// Filtering of Page Parent selection.  
	// This was a required after-the-fact operation for UI display in WP < 2.7 (due to inability to control inclusion of Main Page in UI dropdown)
	...
	function scoper_flt_post_status ($status) {
		...
		*/
		...
		if ( awp_ver( '3.0' ) ) {
			...
			$already_published = in_array( $_post->post_status, array( 'publish', 'private' ) );
		...
		if ( ! $scoper->filters_admin->user_can_associate_main( $post_type ) ) {
			...
			if ( $already_published ) {
				...
				} elseif( in_array( $status, array( 'publish', 'private' ) ) )
					$status = 'draft';
			}
		}
	...
	function scoper_flt_pre_object_terms ($selected_terms, $taxonomy, $args = '') {
		...
		if ( 'post' == $src->name ) { // TODO: abstract for other data sources
