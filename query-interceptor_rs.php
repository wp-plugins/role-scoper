...
{	
	...
	function QueryInterceptor_RS() {
		...
		if ( ! $scoper->direct_file_access ) {
			...
			add_filter('terms_request_rs', array(&$this, 'flt_terms_request'), 50, 3);
		}
	...
	function flt_terms_request($request, $taxonomies, $args = '') {
		...
		if ( ! $reqd_caps_by_otype ) {
			...
			foreach( $taxonomies as $taxonomy ) 
				$reqd_caps_by_otype = array_merge( $reqd_caps_by_otype, $scoper->get_terms_reqd_caps( $taxonomy, '', $is_term_admin ) );   // NOTE: get_terms_reqd_caps() returns term management caps on edit-tags.php, otherwise post edit caps
		...
		if ( ( 'post' == $src_name ) && isset( $reqd_caps_by_otype[ $taxonomies[0] ] ) ) {
			...
			*/
			...
			if ( $ids = $scoper->qualify_terms( $reqd_caps_by_otype[$taxonomies[0]], $taxonomies[0], '', compact('return_id_type') ) ) {	// returns term_id since COL_TAXONOMY_ID_RS is not passed as args['return_id_type']
	...
	function flt_objects_where($where, $src_name, $object_types = '', $args = '' ) {
		...
		log_mem_usage_rs( 'flt_objects_where' );
		...
		foreach ( $status_where as $status_name => $this_status_where) {
			...
			if ( ! empty($duplicate_clause[$status_name]) ) {
				...
				//dump($duplicate_clause);
				//dump($clause_status_vals);
				//dump($status_vals);
	...
	function objects_where_role_clauses($src_name, $reqd_caps, $args = '' ) {	
		...
		log_mem_usage_rs( 'objects_where_role_clauses' );	
		...
		if ( $use_object_roles ) {
			$applied_object_roles = $scoper->get_applied_object_roles($user); // no content date limits involved for object roles
		...
		if ( is_admin() && defined( 'RVY_VERSION' ) && rvy_get_option('pending_revisions') ) {
			if ( empty( $GLOBALS['revisionary']->skip_revision_allowance ) ) {
				$revision_uris = apply_filters( 'scoper_revision_uris', array( 'p-admin/post.php', 'p-admin/page.php', 'p-admin/edit.php', 'p-admin/edit-pages.php', 'p-admin/upload.php', 'p-admin/widgets.php', 'p-admin/admin-ajax.php' ) );
				...
				if ( is_preview() || agp_strpos_any( $_SERVER['SCRIPT_NAME'], $revision_uris ) ) {
					...
					if ( awp_ver( '3.0' ) ) {
						...
						foreach( (array) $object_type as $_object_type ) {
							$strip_capreqs = array_merge( $strip_capreqs, array("edit_published_{$_object_type}s", "edit_private_{$_object_type}s", "publish_{$_object_type}s") );	
							$reqd_caps []= "edit_{$_object_type}s";
						}
					}
		...
		foreach ( $reqd_caps as $cap_name ) {
			...
			if ( $use_object_roles ) {
				...
				$qualifying_object_roles = $scoper->qualify_object_roles($reqd_caps_arg, $object_type, $user, $base_caps_only);
			}
			...
			if ( $alternate_reqd_caps ) { // $alternate_reqd_caps[setnum] = array of cap_names
				foreach ( $alternate_reqd_caps as $alternate_capset ) {
					foreach ( $alternate_capset as $alternate_reqd_caps ) {
						...
						if ( $use_object_roles )
							if ( $alternate_object_roles = $scoper->qualify_object_roles($alternate_reqd_caps, '', $user, $base_caps_only) )
		...
		log_mem_usage_rs( 'objects_where_role_clauses done' );	
	...
	function objects_where_scope_clauses($src_name, $reqd_caps, $args ) {
		...
		log_mem_usage_rs( 'objects_where_scope_clauses' );
		...
		if ( ! $qualifying_roles )
			$qualifying_roles = $scoper->role_defs->qualify_roles($reqd_caps, 'rs', $object_type);
		else
			$qualifying_roles = $scoper->role_defs->filter_roles_by_type($qualifying_roles, 'rs');
		...
		if ( $use_object_roles && ! $qualifying_object_roles ) {
			...
			// Also exclude any roles which have never been assigned to any object
			$base_caps_only = ! $require_full_object_role && ! $this->require_full_object_role;
			$qualifying_object_roles = $scoper->qualify_object_roles($reqd_caps, $object_type, $user, $base_caps_only);
		}
		//---------------------------------------------------------------------------------
		...
		if ( ! $qualifying_roles && $object_type ) {
		...
		*/
					...
					case OBJECT_SCOPE_RS:	// should only exist with nullstring objscope_clause
						if ( $user_qualifies_for_obj_roles ) {
							...
							foreach ( array_keys($scope_criteria[OBJECT_SCOPE_RS]) as $role_type ) { //should be only one
								...
								$objrole_subselect = "SELECT DISTINCT uro.obj_or_term_id FROM $wpdb->user2role2object_rs AS uro WHERE uro.role_type = '$role_spec->role_type' AND uro.scope = 'object' AND uro.assign_for IN ('entity', 'both') AND uro.role_name IN ($role_in) AND uro.src_or_tx_name = '$src_name' $object_roles_duration_clause $u_g_clause $rs_data_clause";
		...
		log_mem_usage_rs( 'objects_where_scope_clauses done' );
	...
	function flt_objects_results($results, $src_name, $object_types, $args = '') {
		...
		if ( ! is_content_administrator_rs() && ( strpos($_SERVER['SCRIPT_NAME'], 'p-admin/edit.php') || strpos($_SERVER['SCRIPT_NAME'], 'p-admin/edit-pages.php') ) ) {
			if ( awp_ver( '3.0' ) ) {
				if ( $post_type_obj = get_post_type_object( $object_type ) )
					$is_hierarchical = $post_type_obj->hierarchical;
			} else {
				$is_hierarchical = ( 'page' == $object_type );	
			}
		...
		if ( $GLOBALS['scoper']->is_front() && empty($this->skip_teaser) ) {
