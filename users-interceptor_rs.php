...
class UsersInterceptor_RS
{
	...
	function users_queryroles ($reqd_caps, $src_name, $object_id = '', $args = '') {
		...
		if ( ! $skip_object_roles && ( $object_id || $any_object ) ) {
			...
			if ( $object_roles = $this->scoper->qualify_object_roles($reqd_caps, $object_type) )
		...
		if ( $object_id ) {
			...
			if ( $objscope_roles ) {
				...
				foreach ( array_keys($roles_wp) as $role_handle ) {
					// this will also have the effect of disqualifying a WP blog role if all of the qualifying RS roles it contains are objscoped.
		...
		if ( $use_term_roles && $src_name && $roles && ! empty($uses_taxonomies) ) {
			...
			$term_roles = $this->scoper->role_defs->filter_roles_by_type($roles, 'rs');
			...
			if ( ! $ignore_strict_terms ) {
				...
				$roles_current = $this->scoper->role_defs->filter_roles_by_type($roles, 'rs');
