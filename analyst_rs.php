...
class ScoperAnalyst {
	...
	function identify_protected_posts( $attachment_id = 0, $attachments = false, $cols = '',  $args = '' ) {
		...
		if ( $use_private_status )
			$role_clause = "AND rs.role_name IN ('post_reader', 'page_reader')";	// if also checking for private status, don't need to check for restriction of private_reader roles
		else
			$role_clause = "AND rs.role_name IN ('post_reader', 'page_reader', 'private_post_reader', 'private_page_reader')";
		...
		if ( $use_term_restrictions ) {
			...
			$term_default_restriction_query = "SELECT rs.role_name FROM $wpdb->role_scope_rs AS rs "
											. "WHERE rs.role_type = 'rs' AND rs.require_for IN ('entity', 'both') AND rs.topic = 'term' AND rs.max_scope = 'term' AND rs.src_or_tx_name = 'category' AND rs.obj_or_term_id = '0' $role_clause";
