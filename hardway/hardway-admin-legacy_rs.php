...
class ScoperAdminHardwayLegacy {
	function flt_dashboard_count_sentence($sentence, $post_type_text, $cats_text, $tags_text, $pending_text, $comm_text = '' ) { 
		...
		if ( !empty($num_posts->publish) ) { // with feeds, anyone can tell how many posts there are.  Just unlink if !current_user_can			// note: Role Scoper mod - include private posts, pages in total
			$post_text = sprintf( __ngettext( '%s post', '%s posts', $num_posts->publish + $num_posts->private ), number_format_i18n( $num_posts->publish + $num_posts->private ) );
			$post_type_texts[] = $can_edit_posts ? "<a href='edit.php'>$post_text</a>" : $post_text;
		}
		if ( $can_edit_pages && !empty($num_pages->publish) ) { // how many pages is not exposed in feeds.  Don't show if !current_user_can
			$post_type_texts[] = '<a href="edit-pages.php">'.sprintf( __ngettext( '%s page', '%s pages', $num_pages->publish + $num_pages->private ), number_format_i18n( $num_pages->publish + $num_pages->private ) ).'</a>';
		}
		...
		if ( $can_edit_pages && !empty($num_pages->draft) ) {
			$post_type_texts[] = '<a href="edit-pages.php?post_status=draft">'.sprintf( __ngettext( '%s draft page', '%s draft pages', $num_pages->draft ), number_format_i18n( $num_pages->draft ) ).'</a>';
		}
		...
		if ( $can_edit_pages && !empty($num_pages->future) ) {
			$post_type_texts[] = '<a href="edit-pages.php?post_status=future">'.sprintf( __ngettext( '%s scheduled page', '%s scheduled pages', $num_pages->future ), number_format_i18n( $num_pages->future ) ).'</a>';
		}
		
		$pending_texts = array();
		if ( current_user_can('publish_posts') && !empty($num_posts->pending) ) {
			$pending_texts[] = '<a href="edit.php?post_status=pending">'.sprintf( __ngettext( '%s pending post', '%s pending posts', $num_posts->pending ), number_format_i18n( $num_posts->pending ) ).'</a>';
			$plural = ( $num_posts->pending > 1 );
		}
		if ( current_user_can('publish_pages') && !empty($num_pages->pending) ) {
			$pending_texts[] = '<a href="edit-pages.php?post_status=pending">'.sprintf( __ngettext( '%s pending page', '%s pending pages', $num_pages->pending ), number_format_i18n( $num_pages->pending ) ).'</a>';
			$plural = ( $num_pages->pending > 1 );
		}
		
		if ( $pending_texts ) {
			if ( 2 == count($pending_texts) )
				$pending_text = sprintf( _c( 'There are %1$s and %2$s awaiting review.|n posts, n pages', 'scoper'), $pending_texts[0], $pending_texts[1]);
			elseif ( $plural )
				$pending_text = sprintf( _c( 'There are %s awaiting review.|n posts', 'scoper'), $pending_texts[0]);
			else {
				...
				//$pending_text = sprintf( _c( 'There is %s awaiting review.|1 post', 'scoper'), $pending_texts[0]);
			}
		}
