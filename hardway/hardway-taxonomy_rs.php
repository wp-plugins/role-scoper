...
if ( awp_ver( '3.0' ) )
	require_once('hardway_rs.php'); // need get_pages() filtering to include private pages for some 3rd party plugin config UI (Simple Section Nav)
else
	require_once('hardway-legacy_rs.php');
...
{	
	...
	function flt_get_terms($results, $taxonomies, $args) {
		...
		log_mem_usage_rs( 'flt_get_terms ' );
		...
		*/
		...
		// if filtering for term management, no need to involve objects in query
		log_mem_usage_rs( 'terms_request_rs ' );
		
		$query = apply_filters( 'terms_request_rs', $query_base, $taxonomies[0], array('skip_teaser' => ! $do_teaser, 'is_term_admin' => $is_term_admin ) );
		
		log_mem_usage_rs( 'terms_request_rs done ' );
		...
		if ( ! empty($exclude_tree) || ($query_base != $query) || $parent || ( 'all' != $fields ) ) {
			...
			log_mem_usage_rs( 'terms query done ' );
		} else
			$terms = $results;
		...
		//
		if ( 'all' == $fields ) {
			log_mem_usage_rs( 'get_term_ancestors' );
			...
			log_mem_usage_rs( 'get_term_ancestors done' );
		...
		if ( $hierarchical && $hide_empty ) {
			log_mem_usage_rs( 'rs_get_term_descendants' );
			...
			log_mem_usage_rs( 'rs_get_term_descendants done' );
		}
		reset ( $terms );
		//
		// === END Role Scoper MODIFICATION ===
		...
		log_mem_usage_rs( 'flt_Get_terms ready to cache' );
		...
		log_mem_usage_rs( 'flt_get_Terms done' );
...
function rs_tally_term_counts(&$terms, $taxonomy, $object_type = '', $args = array() ) {
	global $wpdb, $scoper;
	
	log_mem_usage_rs( 'function rs_tally_term_counts' );
	...
	if ( awp_ver( '3.0' ) ) {
		...
		$post_types = array_keys($src->object_types);
	}
	...
	// note: don't pass in a taxonomies arg because we need to consider restrictions associated with any taxonomy to determine readable objects for terms of this taxonomy objects_request filter
	$args = array_intersect_key( $args, array_flip( array('skip_teaser') ) );

	$object_type = reset($enabled_types);

	//rs_errlog( $taxonomy );
	//rs_errlog( serialize($post_types) );
	
	log_mem_usage_rs( 'rs_tally_term_counts done - objects_req' );
		
	//$args['taxonomies'] = $taxonomy;
	$request = apply_filters('objects_request_rs', $request, $tx->object_source->name, $object_type, $args);

	log_mem_usage_rs( 'rs_tally_term_counts done - objects_req done' );
	
	$results = scoper_get_results($request);
	
	log_mem_usage_rs( 'rs_tally_term_counts done - query done' );
	...
	log_mem_usage_rs( 'rs_tally_term_counts done - items counted' );
	...
	if ( $pad_counts && ScoperAncestry::get_terms_children($taxonomy) ) {
		...
		log_mem_usage_rs( 'get_terms_children done' );
	...
	log_mem_usage_rs( 'children loop done' );
	...
	log_mem_usage_rs( 'tally loop done' );
	...
	log_mem_usage_rs( 'function rs_tally_term_counts done' );
}
