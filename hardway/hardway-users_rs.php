...
class ScoperHardwayUsers {
	...
	function flt_editable_user_ids($query) {
		...
		if ( strpos ($query, "user_id FROM") && strpos ($query, "meta_key =") ) {
			...
			if ( strpos ($query, "user_id FROM $wpdb->usermeta WHERE meta_key = '{$wpdb->prefix}user_level'") ) {
				...
				global $scoper;	
				$object_id = $scoper->data_sources->detect( 'post', 'id' );
				...
				if ( awp_ver('3.0') ) {
					...
					$edit_others_cap = $post_type_obj->cap->edit_others_posts;
				} else {
					$edit_cap = "edit_{$post_type}s";
					$edit_others_cap = "edit_others_{$post_type}s";
				}
 				
				// only modify the default authors list if current user can edit_others for the current post/page
				if ( current_user_can( $edit_others_cap, $object_id ) ) {
					global $scoper, $current_user;

					$users = $scoper->users_who_can($edit_cap, COL_ID_RS, 'post', $object_id );
	...
	function flt_wp_dropdown_users($wp_output) {
		...
		if ( awp_ver('3.0' ) ) {
			...
			$edit_others_cap = "edit_others_{$object_type}s";
		}
		...
		$have_cap = awp_user_can( $edit_others_cap, $object_id, 0, array( 'require_full_object_role' => true ) );
		...
		if ( $have_cap ) {
			...
			$users = $scoper->users_who_can("edit_others_{$object_type}s", COLS_ID_DISPLAYNAME_RS, 'post', $object_id, $args);
		} else {
	...
	function flt_get_editable_authors($unfiltered_results) {
		...
		$have_cap = cr_user_can( $edit_others_cap, $post->ID, 0, array('require_full_object_role' => true ) );

		if ( $have_cap )
			return $scoper->users_who_can( $edit_cap, COLS_ALL_RS);
		else {
			if ( $post->ID ) {
