...
{	
	...
	function flt_get_pages($results, $args = array()) {
		...
		if ( ! empty( $name ) && ( 'parent_id' == $name ) ) {
			...
			if ( ! $post->post_parent && ! $scoper->filters_admin->user_can_associate_main( $post_type ) ) {
				if ( awp_ver( '3.0' ) ) {
					if ( $status_obj = get_post_status_object( $post->post_status ) )
						$is_published = $status_obj->public || $status_obj->private;
				} else
					$is_published = in_array( $post->post_status, array( 'publish', 'private' ) );
		...
		if ( $scoper->is_front() && scoper_get_otype_option('do_teaser', 'post') && scoper_get_otype_option('use_teaser', 'post', $post_type) && ! defined('SCOPER_TEASER_HIDE_PAGE_LISTING') ) {
			...
			// if filtering the page parent drowpdown, don't require status-specific caps (this mimics previous behavior (based on dropdown_pages filter) with WP < 3.0)
			if ( is_admin() ) {
				if ( awp_ver( '3.0' ) ) {
					$post_type_obj = get_post_type_object( $post_type );
					$edit_cap = $post_type_obj->cap->edit_others_posts;
					$associate_cap = "create_child_{$post_type}s";
				} else {
					$edit_cap = 'edit_others_pages';
					$associate_cap = 'create_child_pages';	
				}
				...
				if ( ! empty($post->post_parent) ) {
					$preserve_or_clause = " $wpdb->posts.ID = '$post->post_parent' ";
					$_args['preserve_or_clause'] = array();
					// foreach( get_post_stati( array( 'internal' => false ) ) as $status_name )  // can't use this until we stop using 'published' status key
					foreach ( array_keys( $scoper->data_sources->member_property('post', 'statuses') ) as $status_name )
						$_args['preserve_or_clause'][$status_name] = $preserve_or_clause;
				}
				...
				$_args['force_reqd_caps'][$post_type] = array();
				// foreach( get_post_stati( array( 'internal' => false ) ) as $status_name ) 
				foreach ( array_keys( $scoper->data_sources->member_property('post', 'statuses') ) as $status_name )
					$_args['force_reqd_caps'][$post_type][$status_name] = array( $edit_cap );

				$_args['alternate_reqd_caps'][0] = array( $associate_cap );
			}
		...
		// If no top pages are left in array, relabel next tier as top pages, so Walker doesn't get confused
		$pages_by_id = array();
		foreach( array_keys($pages) as $key )
			$pages_by_id [$pages[$key]->ID] = true;
			
		foreach( array_keys($pages) as $key ) {
			if ( $pages[$key]->post_parent && ! isset( $pages_by_id[ $pages[$key]->post_parent ] ) )
