...
class ScoperAdminHardway_Ltd {
	...
	function act_check_ajax_referer( $referer_name ) {
		if ( 'add-category' ==  $referer_name ) {  // only applies to WP < 3.0
			...
			if ( ! $parent && ! cr_user_can( 'manage_categories', BLOG_SCOPE_RS, 0, array( 'skip_any_term_check' => true ) ) )
				die('-1');	
		} elseif( 'add-tag' == $referer_name ) {
			if ( awp_ver( '3.0' ) ) {
	...
	function flt_last_resort_query($query) {
		// no recursion
		if ( scoper_querying_db() || $GLOBALS['scoper']->cap_interceptor->in_process )
			return $query;
		...
		if ( strpos($query, "ELECT post_status, COUNT( * ) AS num_posts ") && preg_match("/FROM\s*{$posts}\s*WHERE post_type\s*=\s*'([^ ]+)'/", $query, $matches) ) {
			...
			if ( $_post_type ) {
				...
				foreach( $private_stati as $_status )
		 ...
		 )  // don't filter the comment count query prior to DB storage of comment_count to post record
		{
			...
			if ( awp_ver( '3.0' ) ) {
				...
				//	$use_post_types = array_intersect_key( $use_post_types, array( $post_type => true ) );	
				//}
				...
				foreach( $post_types as $_post_type => $type_obj ) {
					...
					foreach ( $post_statuses as $status => $status_obj ) {
						...
						$reqd_caps[$_post_type][$status_name] = array( $type_obj->cap->edit_others_posts, 'moderate_comments' );
						
						if ( $status_obj->private )
							$reqd_caps[$_post_type][$status_name] []= $type_obj->cap->edit_private_posts;
							
						$property = "edit_{$status_name}_posts";
						if ( ! empty( $type_obj->cap->$property ) && ! in_array( $type_obj->cap->$property, $reqd_caps[$_post_type][$status_name] ) )
							$reqd_caps[$_post_type][$status_name] []= $type_obj->cap->$property;
					}
				}
			} else {
				$reqd_caps = array();
