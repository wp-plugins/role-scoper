...
function log_mem_usage_rs( $label, $display_total = true ) {
	...
	$elapsed = round( $time - $last_time, 3 );
	$total_time = round( $time - $start_time, 3 );
		
	if ( $num_bars = intval( $elapsed * 10 ) )
		$bars = str_repeat( '*', intval($num_bars) );
	else
		$bars = '';
		
	if ( strlen($bars) > 2 ) {
		rs_errlog( serialize( debug_backtrace() ) );
		rs_errlog( '' );		
	}
	...
	rs_errlog($label);
	rs_errlog( memory_new_usage_rs() . $total . "\r\n" . " elapsed: " . $elapsed . " total: " . $total_time . " $bars" );
	rs_errlog( '' );
}
}
