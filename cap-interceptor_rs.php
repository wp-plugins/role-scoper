...
{	
	...
	function _flt_user_has_cap($wp_blogcaps, $orig_reqd_caps, $args)	{
		...
		$edit_post_cap = ( awp_ver( '3.0' ) ) ? $object_type_obj->cap->edit_post : 'edit_post';
		
		if ( ! empty( $_POST ) && ( defined( 'SCOPER_CACHE_SAFE_MODE' ) || ( strpos($_SERVER['REQUEST_URI'], 'p-admin/post.php') && ( $args[0] == $edit_post_cap ) ) ) ) {
			$force_refresh = true;
			$tested_object_ids = array();
			$scoped_where_clause = array();
