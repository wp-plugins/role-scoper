...
{	
	...
	function flt_user_has_cap($wp_blogcaps, $orig_reqd_caps, $args)	{
		...
		*/
		...
		global $check_for_pending_rev; //todo: move this flag back to some object
		if ( awp_ver('2.6') && empty($check_for_pending_rev) ) {
			...
			if ( array_intersect( $rs_reqd_caps, $replace_caps) ) {	// don't need to fudge the capreq for post.php unless existing post has public/private status
				if ( strpos($script_name, 'p-admin/edit.php') || strpos($script_name, 'p-admin/edit-pages.php') || ( in_array( get_post_field('post_status', $scoper->data_sources->detect('id', 'post') ), array('publish', 'private') ) ) ) {
		...
		if ( ('edit_others_posts' == $reqd_caps[0]) && ( strpos($script_name, 'p-admin/page.php') || strpos($script_name, 'p-admin/page-new.php') ) ) {
			...
			if ( awp_ver('2.6') ) { // Allow contributors to edit published post/page, with change stored as a revision pending review
				...
				if ( ! awp_metaboxes_started($object_type) && ! strpos($script_name, 'p-admin/revision.php') ) // don't enable contributors to view/restore revisions
					$rs_reqd_caps[$key] = 'edit_pages';
				else
		...
		if ( empty($args[2]) ) {
			...
			if ( 'delete_page' == $args[0] ) {
				if ( ! is_administrator_rs() && scoper_get_option( 'lock_top_pages' ) ) {
		...
		// If an object id was provided, all required caps must share a common data source
		// (object_types array is indexed by src_name)
		...
		if ( 'moderate_comments' == $reqd_caps[0] ) { 
			global $scoper_last_post_id;
			$scoper_last_post_id = $object_id;
		}
	...
	function generate_missing_object_id($required_cap) {
		...
		} else { // calling function is responsible for enhancing performance by calling this function only for appropriate capreqs
		
			/* on the moderation page, edit_post is tested before moderate_comments */
		...
		if ( ! empty( $_POST ) ) {
			...
			if ( $id = $scoper->data_sources->get_from_http_post('id', $src_name) ) {
				if ( $id > 0 )
					return $id;
			} elseif ( strpos( $_SERVER['SCRIPT_NAME'], 'p-admin/async-upload.php' ) ) {
	...
	function user_can_for_any_term($reqd_caps, $user = '') {
				...
				foreach ( $caps_by_op as $this_op_caps ) { // keyed by op_type
					...
					foreach ($src->uses_taxonomies as $taxonomy)
						if ( ! isset($user->term_roles[$taxonomy]) )
							$user->term_roles[$taxonomy] = $user->get_term_roles($taxonomy);

					if ( array_intersect_key($roles, agp_array_flatten($user->term_roles, false) ) )
		...
		// possible todo: could also add these to $user->allcaps so it we don't have to repeat this operation again in same http session
		// ... maybe not, as this is potential perf enhancement, but could lead to confusing results for other code looking at allcaps
	}
