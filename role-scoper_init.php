...
function scoper_get_option( $option_basename, $sitewide = -1, $get_default = false) {
	...
	if ( 'use_post_types' == $option_basename ) {
		...
		if ( empty($default_post_types) || ! did_action('init') ) {
			...
			if ( awp_ver( '3.0' ) ) {
				...
				$default_post_types = array( 'post' => 1, 'page' => 1 );	
			}
		}
		...
		if ( empty($default_taxonomies) || ! did_action('init') ) {
			...
			if ( awp_ver( '3.0' ) ) {
				...
				$default_taxonomies = array( 'category' => 1 );
	...
	/* elseif ( 'lock_top_level' == $option_basename ) {
		static $hierarchical_post_types;
		
		if ( empty($hierarchical_post_types) || ! did_action('init') ) {
			$default_post_types = array();
			
			if ( awp_ver( '3.0' ) ) {
				foreach ( array_diff( get_post_types( array( 'public' => true, 'hierarchical' => true ) ), array( 'attachment' ) ) as $type )
					$hierarchical_post_types[$type] = 1;
			} else {
				$hierarchical_post_types = array( 'page' => 1 );	
			}
		}
		
		$optval = array_merge( $hierarchical_post_types, (array) $optval );
	}
	*/
...
function scoper_get_taxonomy_usage( $src_name, $object_types = '' ) {
	...
	if ( $taxonomies ) {
		...
		if ( 'post' == $src_name ) {
			if ( $use_taxonomies = scoper_get_option( 'use_taxonomies' ) ) // if not stored, default is to use all
