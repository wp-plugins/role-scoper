...
function scoper_default_options() {
	...
	//$def['lock_top_level']['page'] = 0;
	
	// NOTE: scoper_get_option() applies these defaults, but pre-load them for options UI display
	if ( awp_ver( '2.9' ) && ( strpos( $_SERVER['REQUEST_URI'], 'admin.php?page=rs-options' ) || strpos( $_SERVER['REQUEST_URI'], 'admin.php?page=rs-site_options' ) ) ) {
		require_once( 'options-helper_rs.php' );
		$def = supplement_default_options_rs( $def );
	}
...
function scoper_default_otype_options( $include_custom_types = true ) {
	...
	if ( ! awp_ver( '3.0' ) || ! $include_custom_types ) {
		require_once( 'defaults-legacy_rs.php' );
		$def = array_merge( $def, scoper_default_otype_options_legacy() );
		...
		$core_taxonomies = array( 'link_category', 'nav_menu', 'ngg_tag' );

		if ( awp_ver( '3.0' ) )
			$core_taxonomies = array_merge( $core_taxonomies, get_taxonomies( array( 'public' => false ) ) );
...
function scoper_core_data_sources( $include_custom = array() ) {
	global $wpdb;
	...
	'terms_where_reqd_caps' => array(	// Legacy support for WP < 3.0.  note: This criteria is not used to determine the data source, since taxonomy is always supplied to get_terms
		'front' => array(
	...
	'users_where_reqd_caps' => array(	// Legacy support for WP < 3.0  note: A positive URI match can be used to determine data source and cap requirements.
	...
	*/
	...
	if ( ! empty( $include_custom['type'] ) && awp_ver( '2.9' ) ) {
		scoper_add_custom_post_types( $arr );
...
function scoper_core_taxonomies( $include_custom = array() ) {
	$arr = array();
	...
	*/
	...
	if ( ! empty( $include_custom['taxonomy'] ) && awp_ver( '2.9' ) ) {
...
function scoper_core_cap_defs( $include_custom = array() ) {
	$arr = array(
	...
	if ( awp_ver( '2.9' ) ) {
		if ( ! empty( $include_custom['type'] ) )
			scoper_add_custom_post_cap_defs( $arr );
		
		if ( ! empty( $include_custom['taxonomy'] ) )
...
function scoper_core_role_caps( $include_custom = array() ) {
...
//
//note: rs_ is a role type prefix which is required for array key, but will be stripped off for name property
function scoper_core_role_defs( $include_custom = array() ) {
	$arr = array(
	...
	'rs_private_post_reader' =>	(object) array( 'objscope_equivalents' => array('rs_post_reader'),  'object_type' => 'post' ),

	'rs_post_contributor' =>	(object) array( 'objscope_equivalents' => array('rs_post_revisor'),  'object_type' => 'post' ),
	'rs_post_author' => 		(object) array( 'valid_scopes' => array( 'blog' => true, 'term' => true ),  'object_type' => 'post' ),
	'rs_post_revisor' => 		(object) array( 'valid_scopes' => array( 'blog' => true, 'term' => true ),  'object_type' => 'post' ),
	'rs_post_editor' => 		(object) array( 'objscope_equivalents' => array('rs_post_author'),  'object_type' => 'post' ),
	
	'rs_page_reader' => 		(object) array( 'valid_scopes' => array( 'blog' => true, 'term' => true ), 'object_type' => 'page', 'anon_user_blogrole' => true ),
	'rs_private_page_reader' =>	(object) array( 'objscope_equivalents' => array('rs_page_reader'), 'object_type' => 'page' ),

	'rs_page_contributor' =>	(object) array( 'objscope_equivalents' => array('rs_page_revisor'), 'object_type' => 'page' ),
	'rs_page_revisor' => 		(object) array( 'valid_scopes' => array( 'blog' => true, 'term' => true ), 'object_type' => 'page' ),
	'rs_page_author' => 		(object) array( 'valid_scopes' => array( 'blog' => true, 'term' => true ), 'object_type' => 'page' ),
	'rs_page_editor' => 		(object) array( 'objscope_equivalents' => array('rs_page_author'), 'object_type' => 'page' ),
...
function cr_get_reqd_caps( $src_name, $op, $object_type = -1, $status = -1, $base_caps_only = false ) {
	if ( awp_ver( '3.0' ) ) {
		...
		if ( ! $object_type || ( -1 === $object_type ) )
			$object_type = 'post';
		
		if ( ! $status || ( -1 === $status ) )
			$status = 'draft';
		
		if ( $reqd_caps = $scoper->data_sources->member_property( $src_name, 'reqd_caps', $op, $object_type, $status ) ) {
			return $reqd_caps;
		} else
			return array();
	}
