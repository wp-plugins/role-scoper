...
class WP_Scoped_Capabilities extends AGP_Config_Items {
	...
	function get_matching($src_name, $object_type = '', $op_type = '', $status = '', $base_caps_only = false, $args = array() ) {
		$arr = array();
		...
		foreach ( $this->members as $cap_name => $capdef )
			...
			&& ( ! $object_type || ( $object_type == $capdef->object_type ) || ( empty($strict_otype_match) && empty($capdef->object_type) ) )
