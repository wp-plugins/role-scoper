...
class WP_Scoped_Data_Sources extends AGP_Config_Items {
	...
	function get_object($src_name, $object_id, $cols = '') {
		...
		if ( ('post' == $src_name) ) {
			if ( $cols && ! strpos( $cols, ',' ) )
				return get_post_field($cols, $object_id);
			else
	...
	*/
	...
	function get_varname( $var_type, $what_for, $src, $object_type = '') {
		...
		// TODO: track down instances of improperly set properties
		if ( is_object($src) && isset($src->$vars) && is_object($src->$vars) && isset($src->$vars->$what_for) ) {
